...
class Entity:
    ...
    def divisor(self):
        return self.votes / (self.initial_seats + self.extra_seats + 1)
...
def results(competitors: 'list[Apparentment] | list[Party] | list[List]', seats, quota_addition=1):
    total_votes = 0
    ...
    for e in range(extra_seats):
        n = max(competitors, key=lambda i: i.divisor())
        n.extra_seats += 1
    ...
    for ap in apparentments:
        for p in ap.parties:
            for l in p.lists:
                ...
                if l.seats() != 0:
                    print(p.name + " " + str(l.name) + ": " + str(l.seats()) + " (" + str(difference) + ")")
